    ...
    static func handler(client: ClientContainerProtocol) -> @Sendable (Request) async throws -> [RESTContainerSummary] {
        { req in
            ...
            for (container, createdDate) in decorated {
                ...
                // The status string is shared with /system/df through
                // MobyContainerStatus — dockerd renders both surfaces from one builder
                // (daemon/list.go getContainerSummary), and the two had drifted apart here:
                // the list said "Exited (7) 3 seconds ago" while df said "exited".
                // Switch on the state Docker reports, not on the runtime's own status: a
                // container created and never started is `.stopped` to the runtime, and
                // reading that directly printed "Exited (0)" for something that has never
                // run (issue #16).
                let mobyState = container.mobyStateString
                let baseStatus = await MobyContainerStatus.statusString(for: container)
